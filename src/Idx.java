...
/**
 * Idx manages and provides access to Lucene indexes and auxiliary data
 * structures.
 * <p>
 * Most homework assignments only require a single index. However, several
 * distinct indexes can be open simultaneously (e.g., for federated search). The
 * Idx class designates one index the <i>current</i> index. All requests are
 * satisfied from the current index. setCurrentIndex changes the current index.
 * </p>
 */
public class Idx {
	...
	private static HashMap<String, DocLengthStore> openDocLengthStores = new HashMap<String, DocLengthStore>();
	 ...
	 *            Name of attribute
	 * @param docid
	 *            The internal docid in the lucene index.
	 ...
	 * 
	 * @param fieldName
	 *            Name of field to access lengths.
	 * @param docid
	 *            The internal docid in the Lucene index.
	...
	/**
	 * Get the internal document id for a document specified by its external id,
	 * e.g. clueweb09-enwp00-88-09710. If no such document exists, throw an
	 * exception.
	 * 
	 * @param externalId
	 *            The external docid in the Lucene index.
	 * @return iternal docid.
	 * @throws Exception
	 *             Could not read the internal document id from the index.
	 */
	...
	/**
	 * Get the total number of term occurrences contained in all instances of
	 * the specified field in the corpus (e.g., add up the lengths of every
	 * TITLE field in the corpus).
	 * 
	 * @param fieldName
	 *            The field name.
	...
	/**
	 * Get the collection term frequency (ctf) of a term in a field (e.g., the
	 * total number of times the term 'apple' occurs in title fields.
	 * 
	 * @param fieldName
	 *            The field name.
	 * @param term
	 ...
	 * 
	 * @param indexPath
	 *            A directory that contains a Lucene index.
	 ...
	 * 
	 * @param indexPath
	 *            A directory that contains an open Lucene index.
	 * @throws IllegalArgumentException
	 *             The specified index isn't open.
	 */
	...
	/**
	 * return average document length in certain field
	 * 
	 * @param field
	 * @return
	 * @throws IOException
	 */
	public static double getAvgDocLen(String field) throws IOException {
		double docNum = Idx.getDocCount(field);
		if (docNum == 0)
			return -1;
		double sumLen = Idx.getSumOfFieldLengths(field);
		return sumLen / docNum;
	}
}
